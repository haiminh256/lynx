...
bool install_single_package(const std::string& raw_input) {
    ...
    {
        std::lock_guard<std::mutex> lock(install_mutex);
        std::cout << "[Lynx]: Checking package " << package_name << "...\n" << std::flush;
    }
    ...
    try {
        ...
        {
            std::lock_guard<std::mutex> lock(install_mutex);
            std::cout << "[Lynx]: Target version: " << package_name << "@" << target_version << "\n" << std::flush;
        }
        ...
        if (!fs::exists(global_extract_dir, ec)) {
...

            {
                ...
                if (!fs::exists(global_extract_dir, ec)) {
                    ...
                    std::cout << "[Lynx]: Cache hit: " << package_name << "@" << target_version << "\n" << std::flush;
                }
            }
        } else {
            std::lock_guard<std::mutex> lock(install_mutex);
